...
namespace TechnitiumLibrary.Net.Dns
{
    ...
    public class DnsClient : IDnsClient
    {
        ...
        const int MAX_NS_TO_QUERY_PER_REFERRAL = 8; //max NS to query per referral response to mitigate NRDelegationAttack and NXNSAttack
        const int MAX_ASYNC_NS_RESOLUTIONS_PER_REFERRAL = 4; //max NS domain names to resolve async (async NS Resolution feature) per referral response
        const int MAX_OUTBOUND_REQUESTS = 100; //max outbound requests per resolution
        ...
        public static async Task<DnsDatagram> RecursiveResolveAsync(DnsQuestionRecord question, IDnsCache cache = null, NetProxy proxy = null, IPv6Mode ipv6Mode = IPv6Mode.Disabled, ushort udpPayloadSize = DnsDatagram.EDNS_DEFAULT_UDP_PAYLOAD_SIZE, bool randomizeName = false, bool qnameMinimization = false, bool dnssecValidation = false, NetworkAddress eDnsClientSubnet = null, int retries = 2, int timeout = 2000, int concurrency = 2, int maxStackCount = 16, bool minimalResponse = false, bool asyncNsResolution = false, List<DnsDatagram> rawResponses = null, ResolverContext context = null, CancellationToken cancellationToken = default)
        {
                    ...
                    return TaskExtensions.TimeoutAsync(async delegate (CancellationToken cancellationToken1)
                    {
                        ...
                        foreach (string nsDomain in asyncNsResolutionTasks)
                        {
                            if (ipv6Mode != IPv6Mode.Disabled)
                                tasks.Add(RecursiveResolveAsync(new DnsQuestionRecord(nsDomain, DnsResourceRecordType.AAAA, DnsClass.IN), cache, proxy, ipv6Mode, udpPayloadSize, randomizeName, qnameMinimization, dnssecValidation, null, retries, timeout, concurrency, maxStackCount, context: context, cancellationToken: cancellationToken1));

                            tasks.Add(RecursiveResolveAsync(new DnsQuestionRecord(nsDomain, DnsResourceRecordType.A, DnsClass.IN), cache, proxy, ipv6Mode, udpPayloadSize, randomizeName, qnameMinimization, dnssecValidation, null, retries, timeout, concurrency, maxStackCount, context: context, cancellationToken: cancellationToken1));
                        }
            ...
            {
                List<NameServerAddress> newNameServers = new List<NameServerAddress>(ipv6Mode != IPv6Mode.Disabled ? nameServers.Count * 2 : nameServers.Count);
                                        ...
                                        else
                                        {
                                            ...
                                            else
                                            {
                                                ...
                                                while (true)
                                                {
                                                    ...
                                                    nextNameServers = NameServerAddress.GetNameServersFromResponse(cachedNsResponse, ipv6Mode, false);
                                                    InspectCacheNameServersForLoops(nextNameServers);
                ...
                while (true) //resolver loop
                {
                    ...
                    for (; nameServerIndex < referralLimit; nameServerIndex++) //try next server loop
                    {
                        ...
                        if (resolvedNameServers.Count > 0)
                        {
                            ...
                            //remove misconfigured name servers from the list for optimization
                            for (int i = resolvedNameServers.Count - 1; i > -1; i--)
                            {
                                if (resolvedNameServers[i].Metadata.IsMisconfigured)
                                {
                                    ...
                                    if (referralLimit < nameServers.Count)
                                        referralLimit++; //to allow using other names servers up to the limit
                                }
                            }
                            ...
                            if (ipv6Mode != IPv6Mode.Disabled)
                            {
                                //name server with only ipv4 glue requires adding another name server entry to attempt query for ipv6 support
                                ...
                                for (int i = 0; i < nsCount; i++)
                                {
                                    ...
                                    {
                                        //add cloned name server address to allow future IPv6 address resolution if needed
                                        ...
                                        switch (ipv6Mode)
                                        {
                                            case IPv6Mode.Enabled:
                                                ...
                                                i++; //to skip inserted item
                                                nsCount++; //to prevent skipping existing items in list
                                                break;
                                        ...
                                        if ((referralLimit < nameServers.Count) && (referralLimit < MAX_NS_TO_QUERY_PER_REFERRAL))
                                            referralLimit++; //to allow using the added name server
                                    }
                                }
                            }
                        }
                            ...
                            if (ipv6Mode != IPv6Mode.Disabled)
                            {
                                ...
                                else
                                {
                                    if (!wasIPv4Attempted)
                                    {
                                        ...
                                        {
                                            case IPv6Mode.Enabled:
                                                nameServers.Insert(nameServerIndex + 1, currentNameServer); //insert to allow immediate IPv4 address resolution attempt if needed
                                                break;
                        ...
                        try
                        {
                            ...
                            response = await dnsClient.InternalResolveAsync(request, async delegate (DnsDatagram response, CancellationToken cancellationToken1)
                            {
                                ...
                                if (dnsClient._dnssecValidation)
                                {
                                    ...
                                    {
                                        //use new dns client to query the same name server that responded
                                        //this allows updating metadata for success/failure against the same name server
                                ...
                                if ((response.RCODE == DnsResponseCode.NoError) && (response.Answer.Count == 0) && (response.Authority.Count > 0))
                                {
                                    foreach (DnsResourceRecord authorityRecord in response.Authority)
                                    {
                                        if ((authorityRecord.Type == DnsResourceRecordType.NS) && authorityRecord.Name.Equals(currentZoneCut, StringComparison.OrdinalIgnoreCase))
                                        {
                                            ...
                                            if (authorityRecord.Name.Contains('.')) //avoid marking root and TLD name servers
                            ...
                            case DnsResponseCode.NoError:
                                {
                                    ...
                                    else if (response.Authority.Count > 0)
                                    {
                                        ...
                                        else
                                        {
                                            ...
                                            List<NameServerAddress> nextNameServers = NameServerAddress.GetNameServersFromResponse(response, ipv6Mode, true);
        ...
        private static List<NameServerAddress> GetOrderedNameServersToPreferPerformance(IReadOnlyCollection<NameServerAddress> nameServers, bool prioritizeOnesWithIPAddress, IPv6Mode ipv6Mode)
        {
            ...
            if (ipv6Mode == IPv6Mode.Preferred)
                ...
                case IPv6Mode.Enabled:
                    {
                        ...
                        rootHints = ipv6Hints.Interleave(ipv4Hints); //interleave name servers
                    }
                    break;
            ...
            try
            {
                ...
                switch (response.RCODE)
                {
                    case DnsResponseCode.NoError:
                        ...
                        //check authority even if answer section is not empty due to CNAME cases
            ...
            if (concurrency > 1)
            {
                using (CancellationTokenSource cancellationTokenSource = new CancellationTokenSource())
                {
                    await using (CancellationTokenRegistration ctr = cancellationToken.Register(cancellationTokenSource.Cancel))
                    {
                        ...
                        //add delay task
                        Task delayTask = Task.Delay(_timeout * _retries * (int)Math.Ceiling((double)servers.Count / concurrency) * 2, currentCancellationToken); //doubling the actual timeout since the DNS client retries without EDNS on timeout when no DNSSEC validation and does extra DNSKEY requests with DNSSEC validation
                        tasks.Add(delayTask);
