    ...
    public class TorHiddenServiceInfo
    {
        ...
        internal TorHiddenServiceInfo(StreamReader sR)
        {
            while (true)
            {
                ...
                if (response.StartsWith("250-ServiceID=", StringComparison.OrdinalIgnoreCase))
                {
                    _serviceId = response.Substring(14);
                }
                else if (response.StartsWith("250-PrivateKey=", StringComparison.OrdinalIgnoreCase))
                {
                    _privateKey = response.Substring(15);
                }
                else if (response.StartsWith("250-ClientAuth=", StringComparison.OrdinalIgnoreCase))
                {
