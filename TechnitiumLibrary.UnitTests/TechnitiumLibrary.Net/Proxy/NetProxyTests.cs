...
namespace TechnitiumLibrary.UnitTests.TechnitiumLibrary.Net.Proxy
{
    ...
    public class NetProxyTests
    {
        ...
        private static TcpListener StartListener(IPAddress address, out IPEndPoint localEndPoint)
        {
            ...
            Assert.IsInstanceOfType<IPEndPoint>(listener.LocalEndpoint, "Listener.LocalEndpoint must be an IPEndPoint instance.");
        ...
        public async Task ConnectAsync_BypassedEndpoint_UsesDirectTcpAndSkipsProtectedConnect()
        {
            ...
            IPEndPoint proxyEp = new IPEndPoint(IPAddress.Loopback, 65000);
            TestNetProxy proxy = new TestNetProxy(proxyEp);
        ...
        public async Task ConnectAsync_NonBypassedEndpoint_ConnectsToProxyEndpointAndInvokesProtectedConnect()
        {
            ...
            IPEndPoint remote = new IPEndPoint(IPAddress.Parse("203.0.113.77"), 9000);
        ...
        public async Task ConnectAsync_ChainOfProxies_UsesViaProxyThenMainProxy()
        {
            ...
            ChainedNetProxy viaProxy = new ChainedNetProxy(viaProxyEp)
            {
            ...
            IPEndPoint mainProxyEp = new IPEndPoint(IPAddress.Loopback, 60000);
            TestNetProxy mainProxy = new TestNetProxy(mainProxyEp)
            {
                ViaProxy = viaProxy
            };

            // Target endpoint is non-bypassed for mainProxy.
            IPEndPoint target = new IPEndPoint(IPAddress.Parse("203.0.113.44"), 443);
        ...
        {
            IPEndPoint proxyEp = new IPEndPoint(IPAddress.Loopback, 8080);
            TestNetProxy proxy = new TestNetProxy(proxyEp)
            {
            ...
            IPEndPoint bypassed = new IPEndPoint(IPAddress.Parse("192.168.10.5"), 80);
            IPEndPoint notBypassed = new IPEndPoint(IPAddress.Loopback, 80); // not in our custom list
