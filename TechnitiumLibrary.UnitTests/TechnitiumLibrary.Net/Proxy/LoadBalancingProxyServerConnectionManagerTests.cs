...
namespace TechnitiumLibrary.UnitTests.TechnitiumLibrary.Net.Proxy
{
    ...
    public class LoadBalancingProxyServerConnectionManagerTests
    {
        ...
        {
            FakeConnectionManager ipv4Manager = new FakeConnectionManager(AddressFamily.InterNetwork);
            FakeConnectionManager ipv6Manager = new FakeConnectionManager(AddressFamily.InterNetworkV6);

            using LoadBalancingProxyServerConnectionManager manager = new LoadBalancingProxyServerConnectionManager(
                new[] { ipv4Manager },
                new[] { ipv6Manager },
                new[] { DummyConnectivityEndPoint });

            IPEndPoint target = new IPEndPoint(IPAddress.Loopback, 12345);
        ...
        public async Task ConnectAsync_WithIPv6Endpoint_UsesIPv6ConnectionManager_IfSupported()
        {
            ...
            IPEndPoint target = new IPEndPoint(IPAddress.IPv6Loopback, 12345);
        ...
        public async Task ConnectAsync_WithUnspecifiedDomain_BothFamiliesAvailable_UsesOneFamilyConsistently()
        {
            ...
            DomainEndPoint domain = new DomainEndPoint("localhost", 443);
            ...
            Assert.IsInstanceOfType<IPEndPoint>(chosen.LastRemoteEndPoint, "Unspecified domain endpoint must be resolved to an IPEndPoint.");

            IPEndPoint resolved = (IPEndPoint)chosen.LastRemoteEndPoint!;
            Assert.AreEqual(
                chosen.Family,
        ...
        public async Task ConnectAsync_WithUnspecifiedDomain_OnlyIPv4Available_ResolvesToIPv4()
        {
            ...
            DomainEndPoint domain = new DomainEndPoint("localhost", 80);
            ...
            Assert.IsInstanceOfType<IPEndPoint>(ipv4Manager.LastRemoteEndPoint, "DomainEndPoint must be resolved to an IPv4 IPEndPoint when only IPv4 is available.");

            IPEndPoint resolved = (IPEndPoint)ipv4Manager.LastRemoteEndPoint!;
            Assert.AreEqual(
                AddressFamily.InterNetwork,
                resolved.AddressFamily,
        ...
        public async Task ConnectAsync_WithUnspecifiedDomain_OnlyIPv6Available_ResolvesToIPv6_IfSupported()
        {
            ...
            Assert.IsInstanceOfType<IPEndPoint>(ipv6Manager.LastRemoteEndPoint, "DomainEndPoint must be resolved to an IPv6 IPEndPoint when only IPv6 is available.");

            IPEndPoint resolved = (IPEndPoint)ipv6Manager.LastRemoteEndPoint!;
            Assert.AreEqual(
                AddressFamily.InterNetworkV6,
                resolved.AddressFamily,
        ...
        {
            FakeConnectionManager primary = new FakeConnectionManager(AddressFamily.InterNetwork);
            FakeConnectionManager secondary = new FakeConnectionManager(AddressFamily.InterNetwork);
            ...
            IPEndPoint target = new IPEndPoint(IPAddress.Loopback, 8080);
        ...
        {
            FakeConnectionManager v4Primary = new FakeConnectionManager(AddressFamily.InterNetwork);
            FakeConnectionManager v4Secondary = new FakeConnectionManager(AddressFamily.InterNetwork);
        ...
        {
            FakeConnectionManager v4Manager = new FakeConnectionManager(AddressFamily.InterNetwork);
        ...
        #endregion tests – Bind and UDP delegation
        ...
        private sealed class FakeUdpHandler : IProxyServerUdpAssociateHandler
        {
            ...
            {
                SocketReceiveFromResult result = new SocketReceiveFromResult
                {
                    ReceivedBytes = 0,
